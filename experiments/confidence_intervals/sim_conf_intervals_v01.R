library(mvtnorm)
library(synthdid)
setwd('./experiments/confidence_intervals')
source('../../R/sdid_lib.R')
...
N_1 <- 1 
# Number of pre-treat years
T_0 <- 10
# Number of post-treat years
T_1 <- 10
...
# Treatment effect
tau <- 1
# Standard Deviation of Error Variance
sigma <- 2
# What is this? 
rank <- 2
# degree of autocorrelation under stationarity 
rho <- 0.7
# Variance matrix
var <- outer(1:T, 1:T, FUN=function(x, y) rho^(abs(x-y)))
# Design matrix for treatment effect. 
W <- (1:N > N_0) %*% t(1:T > T_0)
